...
class LVQClassifier():
    ...
    space of observed data. In winner-take-all training algorithms one determines, for each data
    point, the prototype which is closest to the input according to a given distance measure.
    The position of this so-called winner prototype is then adapted, i.e. the winner is moved
    closer if it correctly classifies the data point or moved away if it classifies the data point
    incorrectly.
    
    An advantage of LVQ is that it creates prototypes that are easy to interpret for experts in
    the respective application domain. LVQ systems can be applied to multi-class classification
    problems in a natural way. It is used in a variety of practical applications."
    
    Because of the nature of the the winner takes it all nature of the algorithm, to avoid never-winning
    prototypes a bias to the distances is applied.
    
    Two training methods are available. By default the standard LVQ training is used. LVQ2 training scheme is
    used withen the flag LVQ2 is set to True.
    
    Ref.
    
    T. Kohonen. Self-Organizing Maps. Springer, Berlin, 1997.
    
    Martinez, T., Berkovich, G. and Schulten, K.J.: Neural Gas Network for Vector Quantization and its
    Application to Time-Series Prediction. In: IEEE Transactions on Neural Networks, 4, 4, 558- 569 (1993)
    ...
    History:
        ...
        Version 1.1 (5/3/2018) add attributes and methods to improve compatibility with scikit-learn
        Version 1.3 (12/3/2018) add Monte-Carlo probabily errors and assertion tests
        Version 1.4 (21/3/2018) add the neural-gas option and correct bug in the heade
    ...
    bias_decrease_rate : float, optional (default=0.9)
        to avoid not winning prototypes (dead neurons), the distances are modified by a bias
        not winning prototypes will have their distances decreased by bias_decrease_rate
        winning prototypes will have their distances increased by bias_decrease_rate to penalize their success
        
    LVQ2 : boolean (default=False)
        Use the LVQ2 training model:
        - positive match : the winner weights get closer to the training features
        - negative match : the winner weights go away from the training features
                           the closest match weights get closer to the training features
                           
    initial_state : string (default='Normal')
        distribution of the initial prototypes (Normal or Uniform)
        
    sigma : float (default='1')
        standard deviation around the average for the initial distribution if a normal
        distribution is requested. It has no effect if initial_state='Uniform'
        
    algorithm: string (default='LVQ')
        use the LVQ or the Neural Gas algorithm (algorithm='NeuralGas')
        
    lamb: float (default=0.1)
        parameter to control the learning decrease rate exp(-k/(lamb*number_of_prototypes). Used only when
        algorithm='NeuralGas'
    ...
    predict(X)
        Returns the predicted label for X. the fit method has to been run before
    ...
    predict_proba(X)
        Returns the predicted class probability for each instance in the input X
    ...
    def __init__(self, n_components=1, alpha=0.5, decrease_rate = 0.9, epochs=3, random_state=1,
                 p=2, bias_decrease_rate=0.9, LVQ2=False, initial_state='Normal', sigma=1.,
                 algorithm='LVQ', lamb=0.1, verbose=0):
        self.n_components       = n_components
        self.alpha              = alpha
        self.decrease_rate      = decrease_rate # < 1 to avoid learning rate reaching 0
        self.epochs             = epochs
        ...
        self.LVQ2               = LVQ2
        self.initial_state      = initial_state # can be otherwise uniform
        self.sigma              = sigma
    ...
    def train(self, X, y, w, wcl, X_err=None):
        ...
        for epoch in range(self.epochs):
            ...
            learning_rate = self.alpha*(1.-self.decrease_rate*epoch/self.epochs)
            if(self.algorithm == 'NeuralGas'):
                lrate = learning_rate * np.exp (-np.arange(0,n_neurons,1.)/(n_neurons*self.lamb))
                if (lrate.min() < 1e-4):
            ...
            for i in range(nb_train):
                ...
                if (X_err is not None):
                    distances = distances / (1. + np.sqrt(np.sum(X_err[i,:]**2))/n_features)
                ...
                if(self.algorithm == 'NeuralGas'):
                    for k in range(kmax):
                        ...
                        new_w = w[j,:] + (2.*(wcl[j] == wclk)-1.)* lrate[k] * (X[i,:]-w[j,:])
                        if np.all((new_w > min_cl[wclk,:]) &  (new_w < max_cl[wclk,:])):
                            w[j,:] = new_w
                    j = ind[0]
                else:
                    j = ind[0]           # index of the winning prototype/neuron
                    wclj = wcl[j].astype(int)
                    if (wclj == y[i]): # if the winner matches, get closer to that training feature
                        new_w = w[j,:] + learning_rate*(X[i,:]-w[j,:])
                        if np.all((new_w > min_cl[wclj,:]) &  (new_w < max_cl[wclj,:])):
                            w[j,:] = new_w
                    else:
                        new_w = w[j,:] - learning_rate*(X[i,:]-w[j,:]) # the winner is going away
                        if np.all((new_w > min_cl[wclj,:]) & (new_w < max_cl[wclj,:])):
                            w[j,:] = new_w
                        if (self.LVQ2):
                            k = ind[np.argmax(wcl[ind] == y[i])] # find the closest positive loser
                            wclk = y[i].astype(int)
                ...
                if (bias.min() < 1e-2):
                            bias = bias/bias.max()
        return w, wins.astype(int) # return the weights and the number of wins (integer)
        ...
        for i in range(n_components):
            for j in range(nbc):
                if(self.initial_state == 'Normal'):
                    w.append(np.random.normal(avg_cl[j,:],self.sigma*std_cl[j,:])) # random starting points
                else:
                    w.append(np.random.random(n_features)*d_cl[j,:]+min_cl[j,:])  # random starting points
                wcl.append(ycl[j])
        w  = np.array(w) # w = array(n_components*nbc,n_features)
        wcl = np.array(wcl)
        prototypes, self.wins = self.train(X,y,w,wcl, X_err=X_err)
        label.append(wcl)
        self.weights       = np.array(prototypes).reshape(n_components*nbc,n_features)
        self.label_weights = np.array(label).reshape(n_components*nbc)
        return self
    
    def dist(self,u,v):
        return ((abs(u-v)**self.p).sum(axis=1))**(1./self.p)
    ...
    def predict(self, X):
        ...
        for i in range(n_samples):
            pred.append(self.label_weights[np.argmin(self.dist(X[i,:],self.weights))])
    ...
    def predict_proba(self, X):
        ...
        for i in range(n_samples):
            ...
            for label in range(self.n_classes_):
                proba_class[label] = 1./distances[np.argmax(label == label_list)]
    ...
    def predict_proba_MC_error(self, X ,X_err, Number_of_MC_iterations=1000):
        ...
        proba_MC = np.array(proba_MC).reshape(Number_of_MC_iterations,self.n_classes_,n_samples)
    ...
    def score(self, X , y_true, sample_weight=None):
        ...
        if (n_samples != n_true):
            raise ValueError ("The sample and the label number of instances are different.")
        y_pred = self.predict(X)
        score = y_true == y_pred
        return self._weighted_sum(score, sample_weight, normalize=True)

    def _weighted_sum(self, sample_score, sample_weight, normalize=False): # similar to cikit-learn
        if normalize:
